...
MARUBOZU_BODY_PCT = 0.80
PINBAR_BODY_PCT = 0.35
PINBAR_WICK_RATIO = 2.0

PROXIMITY_PCT = 0.0025
WIDE_CPR_MIN_PCT = 0.005
...
def analyze_scalping(symbol: str, name: str,
    ...
    Combines location (PDH/PDL/CPR/EMA) + candle pattern (Marubozu/Pin Bar).
    """
    ...
    candidate_setups: List[Tuple[str, float]] = []
    ...
    if side == "BUY":
        stop_loss = l * (1 - STOP_BUFFER_PCT / 100)
        risk = entry - stop_loss
        if risk <= 0:
            return None
        target = entry + TARGET_RR_MULTIPLE * risk
    else:
        stop_loss = h * (1 + STOP_BUFFER_PCT / 100)
